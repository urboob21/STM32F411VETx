...
#define EE04_ADD_WRITE 0xA0; //1010 0000
		...
		if(r_DataAvailable)
			{
				if(StrCompare(r_cmd,&mode1,1)) //doc all EEPROM
					...
					HAL_I2C_Mem_Read_DMA(&hi2c1,0xA0,0,I2C_MEMADD_SIZE_8BIT,RiData,512);
						...
						//HAL_I2C_Mem_Read(&hi2c1,0XA0,address,I2C_MEMADD_SIZE_8BIT,RiData,5,100);
					}
					
				else if(StrCompare(r_cmd,&mode3,1)) //ghi 1 bytes
					{
						HAL_I2C_Mem_Write_DMA(&hi2c1,0XA0,address,I2C_MEMADD_SIZE_8BIT,r_data,1);
						r_DataAvailable=false;
					}
				else if(StrCompare(r_cmd,&mode4,1)) //ghi 5 bytes
					{
						HAL_I2C_Mem_Write_DMA(&hi2c1,0XA0,address,I2C_MEMADD_SIZE_8BIT,r_data,5);
						r_DataAvailable=false;
					HAL_I2C_Mem_Write(&hi2c1,0XA0,address,I2C_MEMADD_SIZE_8BIT,r_data,5,1000);
					}
			}
  }
...
void HAL_I2C_MemRxCpltCallback(I2C_HandleTypeDef *hi2c)  // read EEPROM ->(DMA)->Ridata -> UART -> OUT
{
	...
	if(hi2c->Instance==hi2c1.Instance)
	{
		...
		{
			memcpy(TxData,STX,1);
			//	memcpy(TxData+1,r_cmd,1);
			//	memcpy(TxData+2,r_address,2);
			memcpy(TxData+1,RiData,sizeof(RiData));
			memcpy(TxData+513,ETX,1);
